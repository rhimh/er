...
async def update_(client, message, _):
    ...
    _update_response_ = "<b>A التحديث الجديد متاح للبوت!</b>\n\n➣ دفع التحديثات الآن</code>\n\n**<u>التحديثات:</u>**\n\n"
    _final_updates_ = _update_response_ + updates
    if len(_final_updates_) > 4096:
        url = await Yukkibin(updates)
        nrs = await response.edit(
            f"<b>A التحديث الجديد متاح للبوت!</b>\n\n➣ دفع التحديثات الآن</code>\n\n**<u>التحديثات:</u>**\n\n[انقر هنا للتحقق من التحديثات]({url})"
        )
    else:
        nrs = await response.edit(
    ...
    if await is_heroku():
            ...
            for x in served_chats:
                try:
                    await app.send_message(
                        x,
                        f"{config.MUSIC_BOT_NAME}أعادت للتو نفسها. آسف للمشاكل.\n\nابدأ التشغيل بعد 10-15 ثانية مرة أخرى.",
                    )
                    ...
                    pass
            await response.edit(
                f"{nrs.text}\n\nتم تحديث البوت بنجاح على هيروكو! الآن ، انتظر لمدة 2-3 دقائق حتى يتم إعادة تشغيل البوت!"
            )
            os.system(
        ...
        for x in served_chats:
            try:
                ...
                pass
        await response.edit(
            f"{nrs.text}\n\nتم تحديث البوت بنجاح! الآن ، انتظر لمدة 1 - 2 دقيقة حتى يتم إعادة تشغيل البوت!"
        )
